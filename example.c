...
unsigned char srcword[]="1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ ";
#define msglen 30
...
int maintest()
{
  ...
  int i;
  int count;
  unsigned char codeword[msglen+NPAR];
  ...
  encode_data(msg, msglen, codeword);
...
#define ML (sizeof (msg) + NPAR)
  ...
  for(i=0;i<=count;i++)
  {
    byte_err(rand(), rand()%(msglen+NPAR), codeword);
  }
